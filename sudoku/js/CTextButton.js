function CTextButton(iXPos,iYPos,oSprite,szText,szFont,szColor,iFontSize,oParentContainer){
    ...
    this.animNotify = function(){
        var iHintSpeed = 55;
        var iRot = 2;
        createjs.Tween.get(_oButton, {loop:true}).
                to({rotation:-iRot},iHintSpeed).to({rotation:0},iHintSpeed).
                to({rotation:iRot},iHintSpeed).to({rotation:0},iHintSpeed).
                to({rotation:-iRot},iHintSpeed).to({rotation:0},iHintSpeed).
                to({rotation:iRot},iHintSpeed).to({rotation:0},iHintSpeed).wait(800)//.call(function(){oParent.animHint();});
    };
